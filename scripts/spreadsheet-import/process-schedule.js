...
module.exports.processSchedule = function(sheet) {
  ...
  hash.update(JSON.stringify({ schedule: data, speakers }));
  const schedule = {
    info: info(hash.digest("hex")),
    schedule: data,
    speakers
  };
...
function info(checksum) {
  const now = new Date();
  const conferenceDay =
    now < Date.parse("Sun Jun 01 2019 00:00:00 GMT+0200 (CEST)") ? 1 : 2;
  return {
