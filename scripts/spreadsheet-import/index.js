...
const getVideoMapping = require("./video-feed").getVideoMapping;
...
async function main(params) {
  ...
  const videoMapping = await getVideoMapping();
    ...
    processedRecords.push.apply(
      ...
      records
        ...
        .map(async function(record) {
          ...
          if (!title) {
            throw new Error(
              `Missing name/title in ${sheetId}: ${JSON.stringify(record)}`
            );
          }
...
function prepPersonMetadata(metadata) {
  ...
  const makeName = newSlug =>
    `/${getFilename(data.name, newSlug)}/${getFilename(
      data.talkTitle || "talk",
      newSlug
    )}.html`;
  ...
  if (data.homepage) {
    let home = data.homepage.replace(/https?:\/\//i, "").trim();
